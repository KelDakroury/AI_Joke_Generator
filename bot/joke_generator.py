...
class AbstractJokeGenerator(ABC):
    ...
    def __call_model(self, model, prompt, n_return_sequences):
        ...
        return [{
            ...
            'text': seq
        } for seq in model.generate(prompt, n_return_sequences) if not profanity.contains_profanity(seq)]
    ...
    def _continue_joke(self, model, promt):
        ...
        joke = None
        while joke is None:
            jokes = self.__call_model(model, model_promt, n_return_sequences=1)
            if jokes:
                joke = jokes[0]
        return joke
